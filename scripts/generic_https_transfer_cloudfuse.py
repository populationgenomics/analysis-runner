...
def main(presigned_url_file_path: str, filenames: bool):
    ...
    assert all({billing_project, dataset, output_prefix})
    names = None
    ...
    batch = get_batch(name=f'transfer {dataset}', default_image='gcsfuse')
    output_path = dataset_path(output_prefix, 'upload')
    bucket_name = output_path.removeprefix('gs://').split('/')[0]
    ...
    for idx, url in enumerate(presigned_urls):
        ...
        output_file_path = output_path.removeprefix(f'gs://') + f'/{filename}'
        ...
        j.cloudfuse(bucket=bucket_name, mount_point=f'/{bucket_name}').command(
            f'curl -L {quoted_url} -o /{output_file_path}'
        )
